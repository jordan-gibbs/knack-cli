...
//!
//! **Once per day.** The first incarnation of this module printed the
//! banner on every invocation until the user upgraded; a field report
//! measured it contaminating a dozen-plus tool outputs in one agent
//! session, including mid-pipeline. The cache now records
//! `last_bannered_at` and the banner re-prints at most every 24h.
//!
//! **Header harvest (cloud).** API responses carry an
//! `x-knack-cli-latest` header (see the FastAPI middleware); the HTTP
//! layer feeds it to [`record_latest`], so cloud-mode users get the
//! version signal with zero extra network calls. The R2 fetch is the
//! fallback for self-host mode and cold caches.
//!
...
struct CacheFile {
    ...
    /// When the banner last actually printed. Throttles it to once per
    /// 24h — an un-upgraded install must not contaminate every command's
    /// stderr (agents parse tool output; every stray line is context).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    last_bannered_at: Option<DateTime<Utc>>,
}
...
/// Print the banner at most once per process AND once per 24h across
/// processes (via `last_bannered_at` in the cache). Intended to be
/// called from `main.rs` after `dispatch` returns so the banner is the
/// last stderr line agents see.
pub fn print_update_banner_once(mode: OutputMode, current: &str) {
    ...
    // Cross-process throttle: skip when the banner already printed
    // within the last 24h. Failure to read/stamp degrades to printing —
    // visible beats silent.
    let mut cache = match read_cache() {
        Some(c) => c,
        None => return,
    };
    if let Some(at) = cache.last_bannered_at {
        if Utc::now().signed_duration_since(at).num_hours() < TTL_HOURS {
            return;
        }
    }
    cache.last_bannered_at = Some(Utc::now());
    let _ = write_cache(&cache);
    let _ = Term::stderr().write_line(&line);
}

/// Feed a latest-version signal harvested from an API response header
/// (`x-knack-cli-latest`). Free for cloud users: no extra request. Only
/// writes when the value is a plausible semver and differs from the
/// cache, so the common path is a no-op.
pub fn record_latest(latest: &str) {
    let latest = latest.trim();
    if parse_triple(latest).is_none() {
        return;
    }
    let prior = read_cache();
    if prior.as_ref().is_some_and(|c| c.latest_version == latest && !is_stale(c)) {
        return;
    }
    let cache = CacheFile {
        checked_at: Utc::now(),
        latest_version: latest.to_string(),
        current_version_at_check: Some(env!("CARGO_PKG_VERSION").to_string()),
        last_bannered_at: prior.and_then(|c| c.last_bannered_at),
    };
    let _ = write_cache(&cache);
}
...
async fn refresh(current: String) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // crate::http::client_builder applies the CLI-wide TLS policy
    // (OS trust store, --cacert, KNACK_CA_BUNDLE) — a raw builder here
    // would make the version check the one request that fails behind
    ...
    let cache = CacheFile {
        ...
        last_bannered_at: read_cache().and_then(|c| c.last_bannered_at),
    };
    write_cache(&cache)?;
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn record_latest_updates_cache_and_preserves_banner_stamp() {
        let (_lock, _dir, path) = isolate();
        let stamp = Utc::now() - chrono::Duration::hours(2);
        let seeded = CacheFile {
            checked_at: Utc::now() - chrono::Duration::hours(30),
            latest_version: "0.7.0".into(),
            current_version_at_check: None,
            last_bannered_at: Some(stamp),
        };
        std::fs::write(&path, serde_json::to_string(&seeded).unwrap()).unwrap();

        record_latest("0.8.0");
        let cache = read_cache().unwrap();
        assert_eq!(cache.latest_version, "0.8.0");
        // The banner throttle stamp must survive a version refresh, or
        // every harvested header would re-arm the banner.
        assert_eq!(
            cache.last_bannered_at.map(|t| t.timestamp()),
            Some(stamp.timestamp())
        );

        // Garbage never overwrites a good cache.
        record_latest("not-a-version");
        assert_eq!(read_cache().unwrap().latest_version, "0.8.0");
    }

    #[test]
    fn banner_prints_once_per_day_across_processes() {
        // Simulates the cross-process throttle: a cache whose
        // last_bannered_at is fresh must suppress; an old stamp must
        // re-arm. banner_line() itself stays pure — the throttle lives
        // in print_update_banner_once, so test the stamp logic the same
        // way it reads it.
        let (_lock, _dir, path) = isolate();
        let mut cache = CacheFile {
            ...
            current_version_at_check: None,
            last_bannered_at: Some(Utc::now() - chrono::Duration::hours(2)),
        };
        std::fs::write(&path, serde_json::to_string(&cache).unwrap()).unwrap();
        // banner_line still returns Some (pure comparison)…
        assert!(banner_line("0.5.0", OutputMode::human()).is_some());
        // …but the fresh stamp means print_update_banner_once would skip:
        let recent = read_cache().unwrap().last_bannered_at.unwrap();
        assert!(Utc::now().signed_duration_since(recent).num_hours() < TTL_HOURS);

        cache.last_bannered_at = Some(Utc::now() - chrono::Duration::hours(25));
        std::fs::write(&path, serde_json::to_string(&cache).unwrap()).unwrap();
        let old = read_cache().unwrap().last_bannered_at.unwrap();
        assert!(Utc::now().signed_duration_since(old).num_hours() >= TTL_HOURS);
    }
